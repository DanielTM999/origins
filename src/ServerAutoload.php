...
final class ServerAutoload extends Autoloader
{
    ...
    function getInterfacesAndParentsFromFile(string $file): array
    {
        ...
        if (preg_match('/^\s*(?:final\s+|abstract\s+)?class\s+(\w+)/mi', $content, $classMatch)) {

            if (preg_match('/extends\s+([^{\s]+)/i', $content, $extendsMatch)) {
                $result['extends'] = trim($extendsMatch[1]);
            }

            if (preg_match('/implements\s+([^{\n]+)/i', $content, $implMatch)) {
                $impls = explode(',', $implMatch[1]);
                foreach ($impls as $impl) {
                    $result['interfaces'][] = trim($impl);
                }
            }
        }
    ...
    private function loadWithDependencies(array $files): array
    {
       $pending = $files;
        ...
        foreach ($pending as $file) {
            $info = $this->getInterfacesAndParentsFromFile($file);

            if (!empty($info['interfaces']) || $info['extends'] !== null) {
                $specialFiles[] = $file;
                continue;
            }
        ...
        $specialFiles = array_unique($specialFiles);
